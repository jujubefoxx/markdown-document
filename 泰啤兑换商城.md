...
商城兑换 --虚拟兑换券--> 商城虚拟商品
商城兑换 --实体商品--> 商城兑换配送类型
商城兑换配送类型 --全国配--> 需要物流发货且可加入购物车 --> 根据配置的信息在商城中展示商品
商城兑换配送类型 --同城配--> 不需要物流发货且不可加入购物车 --> 根据配置的信息在商城中展示商品
活动抽奖 --虚拟兑换券--> 活动抽奖虚拟商品 --> 根据配置的信息把商品加入到活动可选商品中
活动抽奖 --实体商品--> 幸运抽奖目前默认配送方式为全国配 --> 根据配置的信息把商品加入到活动可选商品中
```
...
参与限时秒杀 --> 生成订单到订单列表
普通兑换商品 --> 生成订单到订单列表
生成订单到订单列表 --虚拟商品订单--> 订单状态为交易完成
生成订单到订单列表 --实体商品订单--> 配送类型 --全国配-->  是否已发货
是否已发货 --是--> 订单状态为已发货 --> 用户确认收货 --> 订单状态为交易完成
是否已发货 --否--> 订单状态为待发货 --发货--> 订单状态为已发货 --> 可修改物流单号
配送类型 --同城配 --> 同城配配送类型 --送货上门 --> 是否已接单 --否--> 订单状态为待接单
是否已接单 --是--> 订单状态为待配送 --门店配送--> 用户确认收货
订单状态为待接单 --> 门店是否拒单 --否--> 是否已接单
门店是否拒单 --是--> 订单状态为已拒单
同城配配送类型 --自提 --> 是否已自提 --是--> 订单状态为交易完成
是否已自提 --否--> 订单状态为待自提 --用户自提核销--> 用户确认收货
```
